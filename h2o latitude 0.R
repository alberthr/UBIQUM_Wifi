library(h2o)
library(tidyverse)
library(rpart)
library(caret)
library(Metrics)

#------------------------------------------------------------------------------------------------#

# abro ficheros estandarizados
df <- readRDS("df_mobile_std.rds")
vd <- readRDS("vd_mobile_std.rds")

# o abro los ficheros enteros
df <- readRDS("cleandf.rds")
vd <- read.csv("validationData.csv")
df[df==100] <- NA
vd[vd==100] <- NA

#------------------------------------------------------------------------------------------------#

# selecciono los individuos del edificio 0 y borro waps inservibles
df0 <- df[df$BUILDINGID==0,]
colwap <- which(startsWith(names(df), "WAP"))
waps100 <- sapply(df0[,colwap], function(x) nrow(df0) - sum(is.na(x))) %>% as.data.frame()
#waps100 <- sapply(df0[,colwap], function(x) nrow(df0) - sum(x==100)) %>% as.data.frame()

delcol <- which(waps100$.==0)
df0 <- df0[,-delcol]

# limpio vd
vd0 <- vd[vd$BUILDINGID==0,]

#------------------------------------------------------------------------------------------------#

# H2O PROCESS

# inicializo entorno
h2o.init()
h2o.removeAll()
floor_train_h2o <- as.h2o(x = df0, destination_frame = "floor_train_h2o")
floor_test_h2o <- as.h2o(x = vd0, destination_frame = "floor_test_h2o")



flrf1 <- h2o.randomForest(training_frame = floor_train_h2o,
                          validation_frame = floor_test_h2o, 
                          x=c(1:200),
                          y=202,
                          model_id = "fl_rf_covType_v1", 
                          ntrees = 1000,
                          stopping_rounds = 2,
                          score_each_iteration = T,
                          seed = 1000000)


flrf2 <- h2o.randomForest(training_frame = floor_train_h2o,
                          validation_frame = floor_test_h2o,
                          x=c(1:200),
                          y=202, 
                          model_id = "fl_rf_covType2", 
                          ntrees = 200,   
                          max_depth = 30, 
                          stopping_rounds = 2, 
                          stopping_tolerance = 1e-2,
                          score_each_iteration = T, 
                          seed=3000000) 


flgbm1 <- h2o.gbm(training_frame = floor_train_h2o,
                validation_frame = floor_test_h2o,   
                x=c(1:200),
                y=202, 
                model_id = "fl_gbm_covType1", 
                seed = 2000000)  


flgbm2 <- h2o.gbm(training_frame = floor_train_h2o,
                validation_frame = floor_test_h2o, 
                x=c(1:200),
                y=202,
                ntrees = 20,
                learn_rate = 0.2,
                max_depth = 10,  
                stopping_rounds = 2, 
                stopping_tolerance = 0.01, 
                score_each_iteration = T, 
                model_id = "fl_gbm_covType2", 
                seed = 2000000)


flgbm3 <- h2o.gbm(training_frame = floor_train_h2o,
                validation_frame = floor_test_h2o, 
                x=c(1:200),
                y=202,
                ntrees = 50, 
                learn_rate = 0.1, 
                max_depth = 10, 
                sample_rate = 0.7, 
                col_sample_rate = 0.7, 
                stopping_rounds = 2, 
                stopping_tolerance = 0.01, 
                score_each_iteration = T,
                model_id = "fl_gbm_covType3",
                seed = 2000000) 


h2o.performance(flgbm1, floor_test_h2o)
h2o.performance(flgbm2, floor_test_h2o)
h2o.performance(flgbm3, floor_test_h2o)
h2o.performance(flrf1, floor_test_h2o)
h2o.performance(flrf2, floor_test_h2o)

# gbm3  RMSE = 6.68 / R2 = 0.9560 <- std y NA

pred <- as.data.frame(h2o.predict(flgbm3, floor_test_h2o))
vd0$LAT_PRED <- pred$predict

library(plotly)
plot_ly(vd0, x=~LATITUDE, y=~LAT_PRED, color=~as.factor(FLOOR),
        type = 'scatter', mode = 'markers')


for (i in unique(vd0$FLOOR)) {
    tmp <- filter(vd0, FLOOR==i)
    res <- rmse(tmp$LAT_PRED, tmp$LATITUDE)
    cat(paste(i, "-",res, "\n"))
}

