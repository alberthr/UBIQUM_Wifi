...
hotspots[hotspots==100] <- NA
hotspots[,colwap] <- log(hotspots[,colwap]+105)

mdata <- melt(hotspots, id.vars = c("BUILDINGID","FLOOR", "PHONEID"))
mdata <- mdata[complete.cases(mdata),]

sapply(unique(df$PHONEID), function(x) hist(mdata$value[mdata$PHONEID==x], main=x, breaks = 50))
...
for (i in unique(mdata$PHONEID)) {
    ...
    max <- max(tmp$value)
    min <- min(tmp$value)
    median <- median(tmp$value)
    tmpdf <- data.frame(phone=i, median=median, mean=media, sd=desviacion, min=min, max=max)
...
# guardo trabajo
saveRDS(object = df_standarized, file = "df_mobile_std.rds")
saveRDS(object = mediatotal, file = "mediatotal.rds")
