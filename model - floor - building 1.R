...
flgb <- h2o.gbm(training_frame = train,
                  validation_frame = test, 
                  x=x,
                  y=y,
                  model_id = "bld1_floor1_gbm",
                  ntrees = 50, 
                  learn_rate = 0.1, 
                  max_depth = 10, 
                  sample_rate = 0.7, 
...
h2o.performance(flrf, test)
h2o.performance(flgb, test)


base_models <- list(flgb@model_id, flrf@model_id)
ensemble <- h2o.stackedEnsemble(x = x, y = y, training_frame = train, base_models = base_models)
(perf <- h2o.performance(ensemble, newdata = test))


pred <- as.data.frame(h2o.predict(flgb, test))
...
pred <- as.data.frame(h2o.predict(flgbm1, test))
