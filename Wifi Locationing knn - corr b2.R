...
if (exists("transformo_fichero")) {
    ...
    #plot(new$LATITUDE_PRED, new$LONGITUDE_PRED)
    
    
    #----------------------------------------------------------------------------------------------#   
    
    # REAJUSTO BUILDING 2 A SU PUNTO REAL MAS PROXIMO
    
    # abro fichero con localizaciones totales
    df <- read.csv("./data_frames/mergedData.csv")   # <---------------!!!!!!
    
    # creo df con lon lat totales del train set
    dfll <- df[,c("LONGITUDE","LATITUDE")]
    dfll$id <- rownames(dfll)
    
    # me quedo con la parte lon lat que he predicho
    newll <- new[,c("LONGITUDE_PRED","LATITUDE_PRED")]
    
    # miro cual es el punto mas aproximado y anoto su lat lon y la distancia al punto
    pred <- as.numeric(knn3Train(train = dfll[,1:2], test = newll, cl = dfll$id, k = 1))
    new$LONG_CLOSE <- dfll$LONGITUDE[pred]
    new$LATI_CLOSE <- dfll$LATITUDE[pred]
    new$CLOSEDIST <- sqrt((new$LATI_CLOSE - new$LATITUDE_PRED)^2 + (new$LONG_CLOSE - new$LONGITUDE_PRED)^2)
    
    # sustituyo building2 con los datos mas proximos reales
    new$LATITUDE_PRED[new$BUILDING_PRED==2] <- new$LATI_CLOSE[new$BUILDING_PRED==2]
    new$LONGITUDE_PRED[new$BUILDING_PRED==2] <- new$LONG_CLOSE[new$BUILDING_PRED==2]
    ...
    #write.csv(new, "pruebas.csv", row.names = F)

    finalsubmit <- new[,c("LATITUDE_PRED", "LONGITUDE_PRED", "FLOOR_PRED")]
    
    write.csv(finalsubmit, "knn_soft_corr.csv", row.names = F)    
